...
class UniCon extends Component {
  ...
  loadData = async () => {
    ...
    try {
      ...
      let base_url = `https://swapi.dev/api/${entity}/?page=${pagination.page}`;
      if (search.length > 0) base_url = `https://swapi.dev/api/${entity}/?search=${search}`;
      ...
      if (newState.pagination.total === null || (newState.pagination.total === 1 && data.count > 10)) {
        newState.pagination.total = Math.ceil(data.count / 10)
      }
      if (data.count < 10) {
        newState.pagination = {page: 1, total: 1}
      }
  ...
  handlePaginationChange = (page) => {
    ...
    newState.search = '';
    this.setState({...newState}, async () => await this.loadData());
  };
  ...
  loadAllData = async () => {
    ...
    items.forEach((item, index) => {
      if (item.indexOf("https") === -1) {
        items[index] = item.replace("http", "https")
      }
    })
    ...
    responses.forEach((response, index) => {
      if (response.url.indexOf("https") > -1) {
        responses[index].url = responses[index].url.replace("https", "http")
      }
    })
    let index;
    if (newState.modalData.title) {
      index = newState.data.findIndex(x => x.title === newState.modalData.title)
    } else {
      index = newState.data.findIndex(x => x.name === newState.modalData.name)
    }
      ...
      if (Array.isArray(value)) {
        if (value.length > 0) {
          value.forEach(val => {
            ...
            if (buff_index > -1) {
              if (responses[buff_index].data.name) {
                newValue.push(responses[buff_index].data.name)
              } else {
                newValue.push(responses[buff_index].data.title)
              }
            }
          });
